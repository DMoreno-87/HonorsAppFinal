...
void Nutrition::writeNutritionToFile(ofstream& outFile) const{
    outFile << "Start Date: " << month << "/" << day << "/" << year << endl;

    outFile << "Foods Start" << endl;
    for (const auto& food : foods ) {
        food.writeFoodsToFile(outFile);
    }
    outFile << "Foods End" << endl;
    outFile << "Exercises Start" << endl;
    for (const auto& exercise : exercises) {
        exercise.writeExercisesToFile(outFile);
    }
    outFile << "Exercises End" << endl;

    outFile << "End Date: " << month << "/" << day << "/" << year << endl;
}
